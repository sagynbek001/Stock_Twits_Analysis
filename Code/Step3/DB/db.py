...
def config(filename='../Step3/DB/database.ini', section='postgresql'):
  # create a parser
  parser = ConfigParser()
  # read config file
...
def dropStockPriceTable(ticker):
  try:
    params = config()
    ...
    drop_stock_price_table_query = "DROP TABLE "

    ticker = ticker.lower()
    ticker = ticker.replace('-', '_')
    if 'usd' in ticker:
      ticker = str(ticker[:-4])
      drop_stock_price_table_query += 'price_crypt_' + ticker + ";"
    else:
      drop_stock_price_table_query +='price_stock_' + ticker + ";"

    cursor.execute(drop_stock_price_table_query)
    connection.commit()

  except (Exception, psycopg2.Error) as error:
    print("Failed to drop a stock price table: ", error)
...
def getFilesTable():
  try:
    ...
    query = "SELECT * FROM files_processed"
    ...
    print("Failed to select the files table: ", error)
